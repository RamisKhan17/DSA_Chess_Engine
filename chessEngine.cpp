...
int MAX_DEPTH = 6;
int STARTING_TIME = 30000;                          // In milliseconds
int TIME_INCREMENT = 1000;                          // In milliseconds
searchType SEARCH_FUNC = TT_ITERATIVE_DEEPENING_AW; // Look for enum in Engine.h
...
int main()
{
    ...
    try
    {
        ...
        Engine engine2(board, SEARCH_FUNC, MAX_DEPTH, STARTING_TIME, TIME_INCREMENT);
        // Test move generation
        std::vector<Move> legalMoves = board.generateLegalMoves();
        // Create and run GUI
        ChessGUI gui(board, -1, engine1, engine2);
        gui.run();
    }
