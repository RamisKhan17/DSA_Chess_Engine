...
bool Board::setFEN(const std::string& fen) {
    ...
    castlingRights = 0;
    for (char c : castling) {
        if (c == 'K') castlingRights |= CASTLING_WHITE_KINGSIDE;
        else if (c == 'Q') castlingRights |= CASTLING_WHITE_QUEENSIDE;
        else if (c == 'k') castlingRights |= CASTLING_BLACK_KINGSIDE;
        else if (c == 'q') castlingRights |= CASTLING_BLACK_QUEENSIDE;
    }
...
std::string Board::getFEN() const {
    ...
    if (castlingRights == 0) {
        ...
        if (castlingRights & CASTLING_WHITE_KINGSIDE) fen << 'K';
        if (castlingRights & CASTLING_WHITE_QUEENSIDE) fen << 'Q';
        if (castlingRights & CASTLING_BLACK_KINGSIDE) fen << 'k';
        if (castlingRights & CASTLING_BLACK_QUEENSIDE) fen << 'q';
        fen << ' ';
    }
...
std::vector<Move> Board::generatePseudoLegalMoves() const {
    ...
    for (int square = 0; square < 128; square++) {
        if (square & 0x88) continue;  // Skip off-board squares
...
bool Board::isCheck(int side) const {
    ...
    if (kingSquare < 0) return false;  // King not found (shouldn't happen)
...
bool Board::makeMove(const Move& move) {
    ...
    if (std::abs(move.piece) == 1 || move.capturedPiece != 0) {
        halfMoveClock = 0;
    } else {
        halfMoveClock++;
    }
...
bool Board::isCheckmate() {
    if (!isCheck(sideToMove)) return false;
